...
agent_cmd="eosio-agent -d $agents_dir/ --http-validate-host=false --verbose-http-errors --actor=$agent_actor --nodeos-address=$producers_http --relay-address=$relay_middleware --trace-push-address=$trace_middleware"
